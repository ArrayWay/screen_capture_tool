...
> 说明：当前版本作为正式版发布，直接使用 tag：`v1.1` 即可。
...
`screen_capture_tool` 当前整理为正式发布版本，当前定义为 **正式版 V1.1**。
...
这是一个以“**源码 + 可直接运行 EXE 一起分发**”方式维护的 Windows 本地截图工具正式版本，适合个人本地使用、备份和持续迭代。
...
这是 `screen_capture_tool` 项目当前整理的正式版 Release，当前版本定义为：**正式版 V1.1**。
...
- `ScreenCaptureTool_CN.exe`
- `run_screen_capture_tool.vbs`
- 可选：打包一个压缩包，例如 `screen_capture_tool_v1.1.zip`
